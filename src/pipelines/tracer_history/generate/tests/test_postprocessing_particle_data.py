...
def test_first_and_last_zero_crossing_with_zeros_treated(caplog) -> None:
    """Test the function while treating exact zeros"""
    caplog.set_level(logging.WARNING)
    # yapf: disable
    mock_dist = np.array(
        [
            [8, 7, 6, 5, 3, 1, -1, -2, -3],  # simple case, no zeros
            [4, 3, -1, -2, 1, 3, 0, -1, -2],  # exact zero after crossing
            [1, 1, 0, -1, 1, -1, 1, -1, 1],  # exact zero before crossing
            [1, 2, 3, 4, 5, 6, 7, 8, 9],  # no crossings
            [-4, -3, -2, -1, 0, 1, 2, 3, 4],  # wrong direction
            [4, 3, 2, 1, 0, -1, -2, -3, -4],  # crossing with only 0
            [1, 0, -1, -2, -1, 1, 0, -1, -2],  # two zero-crossings
            [1, -1, -1, 1, 0, -1, 1, -1, -1],  # zero enclosed by normal
        ]
    ).transpose()
    # yapf: enable

    klass = postprocess_particle_data.TimeOfCrossingPipeline
    output = klass._first_and_last_zero_crossing(mock_dist, treat_zeroes=True)
    expected_first = np.array([5, 1, 1, -1, -1, 3, 0, 0])
    expected_last = np.array([5, 5, 6, -1, -1, 3, 5, 6])
    np.testing.assert_array_equal(expected_first, output[0])
    np.testing.assert_array_equal(expected_last, output[1])
...
def test_interpolate_crossing_redshift() -> None:
    ...
    differences = np.array(
        [
            ...
            [5, 6, 1, 2, -3, -4, 2, 0, -1],
            [5, 5, 3, 4, 6, -8, -2, -1, 2],
            [5, 5, 4, 2, 1, 0, -1, -2, -3],  # exact zero-crossing
        ]
    ).transpose()  # yapf: ignore
    indices = np.array([2, 5, -1, 3, 4, 4])  # match actual transitions
